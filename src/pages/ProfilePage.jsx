      ...
      <div className="bg-[#1E1E1E] border border-amber-500/40 rounded-2xl p-8 shadow-[0_0_30px_rgba(245,158,11,0.15)] relative overflow-hidden">
        <div className="flex flex-wrap items-center gap-6 border-b border-zinc-800 pb-6 mb-6">
          ...
          {user?.photoURL ? (
            ...
            <MissMinutesLogo size={64} />
          )}

          <div>
            <span className="text-xs text-amber-500 font-bold uppercase tracking-widest block">TVA CLEARANCE RECORD</span>
            <h1 className="text-2xl font-black text-amber-400">{user?.displayName || 'Agent Mobius'}</h1>
            <p className="text-xs text-zinc-400">{user?.role || 'Senior Temporal Analyst'}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div className="p-4 bg-[#141414] border border-zinc-800 rounded-xl space-y-1">
            ...
            <p className="text-amber-300 font-bold">{user?.email || 'agent.mobius@tva.gov'}</p>
          </div>
          <div className="p-4 bg-[#141414] border border-zinc-800 rounded-xl space-y-1">
            <span className="text-zinc-500 uppercase text-[10px]">TVA AGENT ID</span>
            <p className="text-amber-300 font-bold">{user?.uid || 'tva-agent-007'}</p>
          </div>
          <div className="p-4 bg-[#141414] border border-zinc-800 rounded-xl space-y-1">
            <span className="text-zinc-500 uppercase text-[10px]">TEMPORAL ACCESS LEVEL</span>
