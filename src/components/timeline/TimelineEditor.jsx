    ...
    <div className="space-y-4 font-mono">
      ...
      <div className="flex items-center justify-between p-4 bg-[#1E1E1E] border border-amber-500/30 rounded-xl shadow-[0_0_20px_rgba(245,158,11,0.1)]">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="px-3 py-1.5 bg-[#141414] border border-zinc-800 text-amber-300 rounded-lg text-xs font-bold">
          ...
          {dayEntries.map((entry) => {
            ...
            return (
              <div
                ...
                className="absolute left-3 right-3 p-3 bg-gradient-to-r from-amber-500/20 to-orange-500/10 border-l-4 border-amber-500 rounded-r-lg shadow-[0_0_15px_rgba(245,158,11,0.2)] font-mono group hover:scale-[1.01] transition-transform z-10"
              >
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-amber-300 block">{entry.taskTitle}</span>
                    <span className="text-[11px] text-zinc-400 flex items-center gap-2 mt-0.5">
                      <strong className="text-amber-500">{entry.project}</strong>
                      <span>•</span>
                      <span>{entry.startTime} - {entry.endTime} ({entry.hours}h)</span>
                    </span>
                  </div>
                  <span className="px-2 py-0.5 text-[10px] font-bold bg-amber-500 text-black rounded uppercase">
                    {entry.status}
                  </span>
                </div>
              </div>
